# Card-Tab 书签卡片式管理导航站，进入管理模式可以自由移动书签位置，添加和删除书签，支持自定义网站分类，支持切换暗色主题
## 接下来是本人更新的,之前也是用的一个cfworker,但是那个是静态,这个用kv明显舒服很多,我不会html-css,更新全部由chatgpt辅助完成,唯一缺点是代码太多了,gpt无法读全,包出bug的
##### 、
#### 计划更新:
##### 1、为分类做个目录
##### 2、为卡片加个描述(现在卡片只显示名字和地址,感觉不需要显示地址)
##### 3、为卡片做个修改功能
##### 4、为分类做个排序功能,原作者用的object,顺序排不了,用gpt改会出一堆bug

#### 2024.12.26 更新：
##### 1、重构div结构
##### 2、美化了搜索页
##### 3、统一主题色
##### 4、更换获取网站图标接口,用vercel搭建的,项目地址：([sy-records/GetFavicon](https://github.com/sy-records/GetFavicon))  感谢上述接口作者提供的服务
##### 5、图标预加载
##### 6、搜索页布局已经全部完善
#### 2024.12.05 更新：
##### 1、加入acg随机背景美化,用的是cf图床
##### 2、加入了随机主题色,但是懒得改太多,很多bug
##### 3、为分类加了改名功能(无法理解之前为什么没有)
##### 4、为分类加入收缩功能,并带有缓存记忆
##### 5、将搜索框变为主页,并加入翻页美化
